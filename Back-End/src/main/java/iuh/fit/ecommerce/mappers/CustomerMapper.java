...
public interface CustomerMapper {
    ...
    // ----------------------------------------------------
    // Hàm 1: Có roleName (Dùng khi tạo/login)
    // ----------------------------------------------------
    ...
    // ----------------------------------------------------
    // ✅ HÀM 2: Dùng cho Danh sách (Cần map tất cả trường)
    // ----------------------------------------------------
    @InheritInverseConfiguration(name = "toResponse") // Kế thừa mappings từ hàm trên (không có Context)
    @Mapping(target = "rankingName", source = "ranking.name")
    @Mapping(target = "addresses", source = "addresses")
    @Mapping(target = "roles", ignore = true) // Bỏ qua roles vì không có Context
    CustomerResponse toResponse(Customer customer);

    // ----------------------------------------------------
    // Hàm toCustomer (Chuyển từ Request sang Entity)
    // ----------------------------------------------------
