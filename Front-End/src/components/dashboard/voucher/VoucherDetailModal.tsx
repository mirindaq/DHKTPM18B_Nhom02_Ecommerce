...
export default function VoucherDetailModal({
  ...
  const fetchDetail = async () => {
    ...
    try {
      setLoading(true)
      const response = await voucherService.getVoucherDetail(voucherId, startDate, endDate)
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-2xl font-bold text-blue-600">{detail.voucherCode}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600 mb-1">Tên Voucher</div>
                  <div className="text-lg font-semibold text-gray-900">{detail.voucherName}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600 mb-1">Tổng số lần sử dụng</div>
                  <div className="text-xl font-bold text-green-600">{detail.totalUsage}</div>
                </div>
                <div>
                  ...
                  detail.orders.map((order) => (
                    ...
                    >
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <div className="font-semibold text-blue-600">{order.orderCode}</div>
                          </div>
                        </div>
                        <div className="flex items-start gap-2">
                      ...
                      <div className="mt-3 pt-3 border-t flex justify-between items-center">
                        ...
                        <div className="text-sm text-gray-600">
                          Sau giảm: <span className="font-semibold text-blue-600">
