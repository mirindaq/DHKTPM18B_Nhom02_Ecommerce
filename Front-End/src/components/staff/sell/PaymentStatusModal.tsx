...
import { CreditCard, CheckCircle2, XCircle, Check, RefreshCw, Loader2 } from 'lucide-react'
      ...
      try {
        ...
        if (order) {
          if (order.status === 'PROCESSING' || order.status === 'PENDING') {
            ...
            stopPolling()
            closePaymentWindow()
          } else if (order.status === 'PAYMENT_FAILED' || order.status === 'CANCELED') {
        ...
        <div className="py-6">
          ...
          {paymentStatus === 'waiting' && (
            <div className="text-center space-y-4">
              ...
              <div>
                <p className="text-gray-700 font-medium">Vui lòng hướng dẫn khách hàng quét mã QR</p>
                <p className="text-gray-500 text-sm mt-1">Hệ thống sẽ tự động cập nhật khi thanh toán hoàn tất</p>
              </div>
              {orderId && (
                <div className="bg-gray-100 rounded-lg p-3">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Tổng tiền: <span className="font-bold text-red-600">{formatPrice(totalAmount)}</span>
                  </p>
                </div>
              )}
              <Alert className="bg-yellow-50 border-yellow-200">
                <AlertDescription className="text-yellow-800 text-xs">
                  Không đóng cửa sổ này cho đến khi thanh toán hoàn tất. Trang thanh toán đã được mở ở tab mới.
                </AlertDescription>
              </Alert>
            </div>
          )}
          ...
          {paymentStatus === 'success' && (
            <div className="text-center space-y-4">
              ...
              <div>
                <p className="text-green-700 font-bold text-lg">Thanh toán đã hoàn tất!</p>
                <p className="text-gray-600 text-sm mt-1">Đơn hàng #{orderId} đã được xác nhận</p>
              </div>
          ...
          {paymentStatus === 'failed' && (
            <div className="text-center space-y-4">
              ...
              <div>
                <p className="text-red-700 font-bold text-lg">Thanh toán không thành công!</p>
                <p className="text-gray-600 text-sm mt-1">Giao dịch đã bị hủy hoặc hết thời gian</p>
              </div>
